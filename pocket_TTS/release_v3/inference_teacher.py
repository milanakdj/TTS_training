#!/usr/bin/env python3
"""Nepali Pocket-TTS v3 (24-layer teacher) -- runnable CPU example.
...
Worse: this is the 24-layer teacher, trained to be sampled with guidance
(sample_cfg_coef 2.0), and the pocket-tts inference package has no cfg_coef at
all -- so this script can only ever run it unguided, which is its weakest mode.
Fixing the threshold fixes the length, not the content. See the model card.
"""
import sys
import time
...
CONFIG = "hf://milanakdj/pocket-tts-nepali-24l-teacher-v3/config.yaml"
EOS_THRESHOLD = 1.0
