#!/bin/bash
# Compact health report for the v4 teacher run. Safe to run any time.
set -u
R=/root/tts/TTS_training/pocket_TTS
L=$R/v4_teacher_24l.log
PID=$(cat /tmp/v4_teacher.pid 2>/dev/null || echo 0)

echo "===== v4 teacher health @ $(date '+%Y-%m-%d %H:%M') ====="
...
echo "ckpts   : $(ls /workspace/v4_teacher_24l/checkpoint_*.pt 2>/dev/null | wc -l) kept"
