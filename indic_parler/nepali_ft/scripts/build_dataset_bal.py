...
ap.add_argument("--out", default="/workspace/milan_nepali_parler_ft/data_bal")
ap.add_argument("--neutral-hours", type=float, default=13.0,
                help="cap on neutral hours. The first run used all 41.4 h against "
                     "13.7 h of emotion (82%% neutral) and the model collapsed to "
                     "the neutral mode. Rasa's own recipe is ~10 h neutral against "
                     "1-3 h per emotion, i.e. roughly balanced overall.")
...
# cap neutral so it cannot dominate; emotions are kept in full
EMOSET_ = {"ANGER", "HAPPY", "SAD", "FEAR", "DISGUST", "SURPRISE"}
_rng = random.Random(20260905)
for key in raw:
    emo = [r for r in raw[key] if r.get("style") in EMOSET_]
    neu = [r for r in raw[key] if r.get("style") not in EMOSET_]
    _rng.shuffle(neu)
    budget = args.neutral_hours * 3600 * (0.1 if key == "validation" else 1.0)
    kept, acc = [], 0.0
    for r in neu:
        if acc >= budget: break
        kept.append(r); acc += float(r.get("duration") or 0)
    raw[key] = emo + kept
    _rng.shuffle(raw[key])
    eh = sum(float(r.get("duration") or 0) for r in emo) / 3600
    print(f"{key}: emotion {len(emo)} clips/{eh:.1f} h + neutral {len(kept)} clips/{acc/3600:.1f} h", flush=True)
